...
def test_invalid_student_username(page, student_login):
    ...
    page.click("//span[contains(.,'Log In')]")
    assert_that(page.is_visible(
        "//h3[contains(.,'Email or Password is incorrect.')]")).is_true()
...
def test_blank_student_username(page, student_login):
    ...
    page.click("//span[contains(.,'Log In')]")
    assert_that(page.is_visible(
        "//div[@class='errorMessage'][contains(.,'Email is required')]")).is_true()
...
def test_blank_student_password(page, student_login):
    ...
    page.click("//span[contains(.,'Log In')]")
    assert_that(page.is_visible(
        "//div[@class='errorMessage'][contains(.,'Password is required')]")).is_true()
...
def test_blank_student_login(page, student_login):
    ...
    assert_that(page.is_visible(
        "//div[@class='errorMessage'][contains(.,'Password is required')]")).is_true()
    assert_that(page.is_visible(
        "//div[@class='errorMessage'][contains(.,'Password length should be 10 or more.')]")).is_true()
...
def test_student_password_too_long(page, student_login):
    ...
    page.click("//span[contains(.,'Log In')]")
    assert_that(page.is_visible(
        "//div[@class='errorMessage'][contains(.,'Exceeded Number of Characters Allowed')]")).is_true()
...
def student_login(page):
    ...
    page.is_visible(
        "//div[@class='content'][contains(.,'For Teachers and For Students')]")
    page.click("//button[contains(.,'Log In')]")
